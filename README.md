...
3. Deploy `dist/` to any static host (Cloudflare Pages / Netlify / Vercel / cPanel).
   - **Contact form**: powered by [Netlify Forms](https://docs.netlify.com/forms/setup/) (free tier: 100 submissions/month) — submissions appear in the Netlify dashboard under **Forms → Enquiry** and are emailed via the notification configured in **Form settings → Form notifications** (recipient: `dattatrayshinde435@gmail.com`). Spam protection: honeypot field + Akismet. If you ever move off Netlify, switch the submit handler in `src/pages/contact.astro` to another provider (Web3Forms/Formspree).
